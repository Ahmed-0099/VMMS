...
Scope: add, view, search/filter, detail, edit, and delete/soft-delete vehicles.
...
- Create `backend/src/controllers/vehicleController.js`.
  ...
  - `deleteVehicle(req, res, next)`
    - For Phase 1, prefer delete only if no related records.
...
- Add auth/role protection after Auth is complete:
  - `ADMIN`: full access.
  - `TECHNICIAN`: read access if needed.
  - `DRIVER`: only assigned vehicle view later.
